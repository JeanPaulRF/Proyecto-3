...
SELECT
	T.Item.value('@Id', 'INT'),
	T.Item.value('@Nombre', 'VARCHAR(64)')
FROM @xmlData.nodes('Datos/TipoEventos/TipoEvento') as T(Item)
...
SELECT
	T.Item.value('@Id', 'INT'),
	T.Item.value('@Descripcion', 'VARCHAR(64)')
FROM @xmlData.nodes('Datos/TipoMovimientosCO/TipoMovimientoCO') as T(Item)


INSERT INTO [dbo].[TasaInteres]([ID], [TasaInteres])
SELECT
	T.Item.value('@Id', 'INT'),
	T.Item.value('@TasaInteres', 'FLOAT')
FROM @xmlData.nodes('Datos/TasaInteresesCO/TasaInteresCO') as T(Item)